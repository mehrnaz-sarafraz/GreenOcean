...
export default function CommunityDetail() {
  ...
  useEffect(() => {
    ...
    apiRequest<PageResponse<PostItem>>(
      `/api/v1/communities/${community.id}/posts?size=50`,
    )
      .then(result => {
        if (cancelled) {
          return;
  ...
  async function join() {
    ...
    const updated =
      await apiRequest<typeof community>(
        `/api/v1/communities/${community.id}/membership`,
        {
          method: 'PUT',
        },
      );
  ...
  return (
    ...
    >
      <View style={styles.content}>
        ...
        {!community.member && (
          <Text style={styles.empty}>
            Join this circle to read and share
            community posts.
          </Text>
        )}
        ...
        {community.member &&
          ...
          !error && (
            <Text style={styles.empty}>
              No stories have been shared here yet.
            </Text>
          )}
      </View>
    </Screen>
